...
module.exports = function (app) {
  ...
  app.route('/api/issues/:project')
    ...
    .get(function (req, res){
      ...
      var lookFor2 = Object.keys(query).reduce(function(obj,k){
        if(query[k] !== '') obj[k] = query[k];
        return obj;
      },{});
    
      //console.log(lookFor2.open);
      //console.log(lookFor2.hasOwnProperty('_id'))
    
      if(lookFor2.open === 'false') lookFor2.open = false;
      if(lookFor2.open === 'true') lookFor2.open = true;
      if(lookFor2.hasOwnProperty('_id')) lookFor2._id = ObjectId(lookFor2._id);
    
      console.log(lookFor2);
    
      MongoClient.connect(CONNECTION_STRING, function(err,db){
        db.collection(project).find(lookFor2).toArray((err,docs)=>{
    ...
    .put(function (req, res){
      ...
      var updatedEntry = Object.keys(body).reduce(function(obj,k){
    ...
    .delete(function (req, res){
      ...
      MongoClient.connect(CONNECTION_STRING, function(err,db){
        db.collection(project).findOneAndDelete({_id: ObjectId(_id)},(err, docs)=>{
          ...
          if(docs === null){
            res.type('text').send('deleted 1 ' + _id);
          } else res.type('text').send('deleted 2 ' + _id);
